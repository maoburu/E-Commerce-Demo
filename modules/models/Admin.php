...
class Admin extends ActiveRecord
{
    ...
    {
        return [
            ['adminuser', 'required', 'message' => '管理员账号不能为空', 'on' => ['login', 'seekpassword', 'changepass', 'adminadd', 'changeemail']],
            ...
            ['rememberMe', 'validatePass', 'on' => 'login'],
            ['adminemail', 'required', 'message' => '电子邮箱不能为空', 'on' => ['seekpassword', 'adminadd', 'changeemail']],
            ['adminemail', 'email', 'message' => '电子邮箱格式不正确', 'on' => ['seekpassword', 'adminadd', 'changeemail']],
    ...
    public function validateEmail()
    {
        if(!$this->hasErrors()) {
            ...
            if(is_null($data)) {
                $this->addError("adminemail", "管理员账号与邮箱不匹配");
            }
        }
    }
    ...
    public function login($data)
    {
        ...
        if($this->load($data) && $this->validate()) {
            ...
            $this->updateAll([
                'logintime' => time(),
                'loginip' => ip2long(Yii::$app->request->userIP == '::1'?'127.0.0.2':Yii::$app->request->userIP)
                ],
